...
export const AGRESSION_CHANGE_MESSAGE_PROMPT = (
  ...
  Контекст: Выявлено, что пользователь ${analyzeResult.needChange} ведет себя конфликтно.
  Целевые ID для переработки: ${toxicIdsStr}.

  ТВОЯ ЗАДАЧА:
  1. Сформулируй "очищенную" суть сообщений из списка ID выше, используя «Я-сообщения».
  2. ПРАВИЛО ТИШИНЫ: Если в этих сообщениях НЕТ никакого смысла, кроме оскорблений (например, просто ругань, маты или фразы типа "ты дебил"), то НИЧЕГО НЕ ПИШИ. В ответе должна быть абсолютно пустая строка.
  3. Не повторяйся: Твой ответ не должен дублировать по смыслу то, что уже написано в сообщениях с пометкой "isHistoryOnly: ТОЛЬКО ИСТОРИЯ". Если смысл остался тот же, то ничего не отвечай
